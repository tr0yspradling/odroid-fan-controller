...
def status():
    print
    "Fan Speed: {0}%".format(getfanspeed())
    print
    "Fan Mode: {0}".format(getfanmode(str=True))
...
def setfanspeed(speed):
    ...
    if getfanmode(str=True) == 'manual':
        if speed < 25:
            print
            'Fan power below 25% causes a malfunction with the odroid fan. Returning.'
            return
...
def setfanmode(fanmode):
    ...
    if (_mode == 'auto'):
        ...
        _mode = '0'
    print
    '_mode: {0}'.format(_mode)
    if (_mode == '1') or (_mode == '0'):
        print
        'getfanmode(str=False): {0}'.format(getfanmode(str=False))
        if getfanmode(str=False) == _mode:
            print
            'Fan mode already set to {0}'.format(fanmode)
            return
        writetofile(odroid_fan_mode_file, _mode)
        print
        '_mode: {0} written to {1}'.format(_mode, odroid_fan_mode_file)
        print
        'getfanmode(str=False)(after write): {0}'.format(getfanmode(str=False))
    else:
        print
